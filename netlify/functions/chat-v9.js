// ============== DIRCBOT NETLIFY FUNCTION v9 ==============
// CHANGES FROM v8:
//   + Accepts tester identification fields: testerCode, testerEmail, testerName
//   + Logs every successful chat turn to Netlify Form "dirc-messages" (fire-and-forget)
//   + Logs model_label, tokens, truncated question/answer for admin dashboard
//   Everything else unchanged: KB loading, prompt caching, memory protocol, multimodal.
//
// Accepts the app.js payload:
//   { message, conversationHistory[], language, topic?, fileData?, fileType?, fileName?,
//     userContext?, userMemories?, projectContext?,
//     testerCode?, testerEmail?, testerName? }      // <-- NEW for tracking
  ...
  pdfParsePromise = (async () => {
    ...
    try {
      ...
      for (const file of files) {
        ...
        combined += `\n\n## File: ${file}\n${content}`;
        perFile.push({ file, type: 'md', source: 'repo', tokens, chars: content.length });
      }
    } catch (err) {
      ...
      try {
        ...
        for (const file of pdfFiles) {
          if (usedTokens >= KB_TOKEN_BUDGET) {
            perFile.push({ file, type: 'pdf', source: 'repo', skipped: true, reason: 'budget' });
            continue;
          }
          try {
            ...
            if (!text) {
              perFile.push({ file, type: 'pdf', source: 'repo', skipped: true, reason: 'empty' });
              continue;
            }
            ...
            perFile.push({
              file, type: 'pdf', source: 'repo',
              tokens: estimateTokens(textToAdd), chars: textToAdd.length, pages: data.numpages
            });
          } catch (pdfErr) {
            perFile.push({ file, type: 'pdf', source: 'repo', skipped: true, reason: pdfErr.message });
          }
        }
      } catch (err) {
    ...
    if (getStoreFn && blobsInitialised) {
      try {
        ...
        console.warn('Blob KB unavailable:', err.message);
      }
    }
...
// ============== NEW IN v9: TRACKING / LOGGING ==============
// Fire-and-forget: posts each chat turn to Netlify Form "dirc-messages".
// Never blocks the response — wrapped in try/catch, errors are warnings only.
//
// Logged fields:
//   email, code, user_name (tester identification)
//   model_label (e.g. "claude-sonnet-4-6")
//   question, answer (truncated to 500 chars each)
...
async function logChatToNetlify({ testerCode, testerEmail, testerName, modelLabel, question, answer, usage, siteUrl }) {
  // Only log if we have at least a tester code — anonymous chats are not tracked
  if (!testerCode) return;

  try {
    const formData = new URLSearchParams({
      'form-name': 'dirc-messages',
      email: testerEmail || '',
      code: testerCode || '',
      ...
      tokens_in: String(usage.inputTokens || 0),
      tokens_out: String(usage.outputTokens || 0),
      cache_read: String(usage.cacheReadTokens || 0),
      cache_create: String(usage.cacheCreateTokens || 0),
      timestamp: new Date().toISOString()
    });

    // Target the site root for Netlify Forms submission
    const target = siteUrl || process.env.URL || process.env.DEPLOY_URL || 'https://dircbot.netlify.app';
    ...
    if (!resp.ok) {
      console.warn(`[tracking] log post returned ${resp.status} for ${testerCode}`);
    } else {
      console.log(`[tracking] logged turn for ${testerCode} (${usage.outputTokens || 0} out tokens)`);
    }
  } catch (err) {
    // Logging must never break the response — just warn
    console.warn('[tracking] logChatToNetlify error (non-blocking):', err.message);
  }
}
...
exports.handler = async (event) => {
  ...
  try {
    ...
    // NEW v9: tester identification for tracking (all optional — bot works without)
    const testerCode = (body.testerCode || '').toString().trim();
    const testerEmail = (body.testerEmail || '').toString().trim();
    const testerName = (body.testerName || '').toString().trim();
    ...
    // NEW v9: Fire-and-forget tracking log
    // Awaited briefly so the Lambda doesn't terminate before fetch is sent,
    // but failures are caught and never break the response.
    if (testerCode) {
      // We strip the MEMORY block from the logged answer so the admin dashboard
      // shows what the user actually saw, not the hidden directive.
      const visibleAnswer = replyText.replace(/<<MEMORY>>[\s\S]*?<<END_MEMORY>>/g, '').trim();

      // Use siteUrl from event headers if available — more reliable than env vars in some setups
      const siteUrl = event.headers?.host
        ? `https://${event.headers.host.replace(/^https?:\/\//, '')}`
        : null;

      await logChatToNetlify({
        testerCode,
        testerEmail,
        testerName,
        modelLabel: response.model || 'claude-sonnet-4-6',
        question: message,
